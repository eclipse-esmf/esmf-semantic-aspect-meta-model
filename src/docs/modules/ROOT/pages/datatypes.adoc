...
xref:bamm-specification:appendix:appendix.adoc#xmlschema11-2[[xmlschema11-2\]]), including the types
such as data ranges. In addition to types from XSD, the type `langString` is included as described
in the RDF xref:bamm-specification:appendix:appendix.adoc#rdf11[[rdf11\]] specification; it is used
