...
with or without time zone. No `value` Property is defined, hence the semantics of the value can be
defined in the specific Aspect Model.
...
Note that an Abstract Entity can have both `bamm:Property`s and `bamm:AbstractProperty`s. The
properties list of an Entity that extends the Abstract Entity must specify the Characteristic to use
for each of the Abstract Properties (and only those).
