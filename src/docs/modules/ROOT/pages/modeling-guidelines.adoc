...
|===
| Attributes | Description | Required
| `bamm:properties` | The list of Properties of this Aspect. Leaving out this
  attribute completely is equivalent to having it present with an empty list as value. | {nok}
| `bamm:operations` | The list of Operations of this Aspect. Leaving out this
  attribute completely is equivalent to having it present with an empty list as value. | {nok}
...
NOTE: `bamm:notInPayload` cannot be used in Abstract Entities, but the setting for a specific property
...
|===
| Attributes | Description | Required
| `bamm:properties` | The list of Properties which make up the Entity. Leaving out this
  attribute completely is equivalent to having it present with an empty list as value.| {nok}
...
The effective Property names, i.e. the payload name when `bamm:payloadName` is used, or the name of the Property,
must be unique across the entire Entity inheritance chain. Otherwise the corresponding runtime payload is ambiguous.

When one of the attributes xref:modeling-guidelines.adoc#payload-names[bamm:payloadName],
xref:modeling-guidelines.adoc#optional-properties[bamm:optional] or `bamm:notInPayload` is used on a specific property usage,
it's value is also inherited by the extending Entities. Using the example of `bamm:payloadName`,
...
[source,turtle,subs="attributes+"]
----
include::example$aspect-sample.ttl[tags=extending-entity-declaration-inheritance]
----
...
Property can be declared a `bamm:AbstractProperty`. An Abstract Property is similar to a Property,
with two differences: Firstly, it has no `bamm:Characteristic` attribute and secondly, it must only
...
Note that an Abstract Entity can have both `bamm:Property`&#8203;s and
`bamm:AbstractProperty`&#8203;s (both are listed in `bamm:properties`). The properties list of an
...
NOTE: The setting of the `bamm:optional` attribute for a specific property is inherited from parent Entity
by the Entities extending it using the Entity extension mechanism (`bamm::extends`).
...
by it's children when using the Entity extension mechanism (`bamm::extends`).
...
[source,turtle,subs="attributes+"]
----
include::example$localization-samples.ttl[tags=multi-lang-example]
----
