...
Note that an Abstract Entity can have both `bamm:Property`s and `bamm:AbstractProperty`s (both are
listed in `bamm:properties`). The properties list of an Entity that extends the Abstract Entity must
specify the Characteristic to use for each of the Abstract Properties (and only those).
...
To achive this, we need to change the type of the `description` property to the Set Characteristic:
