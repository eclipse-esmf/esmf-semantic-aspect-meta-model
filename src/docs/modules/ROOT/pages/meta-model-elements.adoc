...
image::aspect-meta-model-simplified.svg[width=100%]
...
* A _Constraint_ is a special type of Characteristic that acts as a constraint (for example on the
  value range) on another Characteristic, which is referred to as the _Base Characteristic_.

Characteristics, just like model element descriptions, are _not part of the data_ an Aspect sends,
but only exist in its model. A number of commonly used Characteristics are predefined in the scope
of the {meta-model-full-name}, see section
