...
`"Could not load data"@en` would be serialized in the JSONpayload as follows:
[source,json]
----
{
  "errorMessage": {
...
A Property `errorMessages` with a Collection Characteristics and effective data type `rdf:langString` could yield the payload:
[source,json]
----
{
  "errorMessages": [
