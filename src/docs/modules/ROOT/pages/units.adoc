...
| `bamm:name` | Unique name of quantity kind | {ok}
...
| `bamm:name` | Unique name of unit | {ok}
| `bamm:preferredName` | Human readable name in a specific language | {ok}
| `unit:commonCode` | Normalized short code for unit; please refer to the
...
| `unit:conversionFactor` | If the unit is derived from a reference unit, the _human readable_ multiplication factor,
  e.g., "10⁻²⁸ m²" | {nok}
| `unit:numericConversionFactor` | If the unit is derived from a reference unit, the _numeric_ multiplication factor,
  e.g., "1.0E-28" | {nok}
| `unit:quantityKind` | The list of quantity kinds, for example unit _litre_ has quantity kind
  _volume_, unit _metre_ has quantity kinds _length_, _distance_, _diameter_ etc. | {nok}
| `unit:referenceUnit` | The unit this unit is derived from, e.g., _centimetre_ is derived from
  _metre_| {nok}
| `unit:symbol` | The unit's symbol, e.g., for _centimetre_ the symbol is _cm_ | {nok}
|===
