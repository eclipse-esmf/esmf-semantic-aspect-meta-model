...
When multiple Constraints are defined in a single `samm-c:Trait`, **all Constraints MUST be
fulfilled simultaneously**.

In other words, Constraints within a Trait are combined using logical AND.
The value of a Property is valid only if it satisfies **every** Constraint declared in the Trait.

The specification does not support OR semantics between Constraints within a Trait.
If alternative constraint sets are required, they must be modeled explicitly, for example
by using separate Properties or an `Either` Characteristic.
...
The `samm:dataType` of a `samm:Value` is implicitly defined by the type of the literal or reference assigned to `samm:value`.
For example, in the case above where `samm:value "green"` is used, the data type is `xsd:string`.
If the value is an integer (e.g., `samm:value 42`), the data type will be `xsd:integer`.
