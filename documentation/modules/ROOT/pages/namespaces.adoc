...
* The prefix "samm" shall be used for the URN `{samm}`
* The prefix "samm-c" shall be used for the URN `{samm-c}`
* The prefix "samm-e" shall be used for the URN `{samm-e}`
...
* For other elements, use prefixes that do not contain the string `samm`.
...
[[describe-namespace-in-an-Aspect]]
== Describe Namespace in an Aspect

Currently there is official way to provide a human-readable name and a description for namespaces.
This is supported for general Meta-Model definitions like Aspects, Entities, etc.
A name and description allows users to better understand the intended use (provide further context information/meaning).

For example, if a namespace provides a list of Events and is intended to be used as definition library or in case of Operations, for which use case the operations are intended for, e.g. for a forklift, or a car multimedia system.
