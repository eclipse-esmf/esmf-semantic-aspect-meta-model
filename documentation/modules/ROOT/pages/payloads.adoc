...
* A Property's _trait Characteristic_ means the Property's Characteristic if it is not a Constraint,
  otherwise it means transitively following the Constraint's `samm-c:baseCharacteristic` until the
  value is not a Constraint any more.
* A Property's _effective data type_ means the Property's trait Characteristic's `samm:dataType`.
...
** If the Property's trait Characteristic is a
