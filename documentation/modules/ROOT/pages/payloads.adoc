...
IMPORTANT: It is necessary to clarify, that sample payload may be invalid.

Here are a few points that you should consider when designing Aspect Models:
====
* Aspect Models are permitted even when no valid payload can exist;
* The sample payload generator may not consistently produce a valid payload even if one exists;
* The generated payload data result can be invalid.
====

See an example of a similar case below.
