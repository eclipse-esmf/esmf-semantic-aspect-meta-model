...
Describes a unique type of property within the ESMF .
A Reference can either be a global reference (e.g., a URN) or a reference to a property
of another aspect within a different twin.
...
|===
| Attributes | Description | Required
| `samm-c:referenceType` | The `samm-c:referenceType` is set to a URN, but it could be any value,
