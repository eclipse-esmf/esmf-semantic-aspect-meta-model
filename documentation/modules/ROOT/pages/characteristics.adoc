...
* Creating a Model with a <<set-characteristic>>/<<sorted-set-characteristic>> based on a finite Base Characteristic (`https://eclipse-esmf.github.io/samm-specification/snapshot/characteristics.html#boolean-characteristic[Boolean]` or `https://eclipse-esmf.github.io/samm-specification/snapshot/characteristics.html#enumeration-characteristic[Enumeration]`)
and adding Length Constraint, the `samm-c:minValue` should not exceed the total number of elements in the Base Characteristic.
Models with such conditions are not recognized as invalid, but for which no payload can exist.
The responsibility is borne by the user who created these models with such Length Constraints for the characteristics.
