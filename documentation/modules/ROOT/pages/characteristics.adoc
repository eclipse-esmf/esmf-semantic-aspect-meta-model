...
IMPORTANT: A Property that uses an `Either` Characteristic must not define an `exampleValue`.

An `xref:modeling-guidelines.adoc#declaring-properties[exampleValue]` represents a concrete example payload.
For an `Either`, the concrete value shape depends on whether the *left* or the *right*
Characteristic is used. Since `Either` itself does not define a single, well-defined
`samm:dataType`, an `exampleValue` defined at the Property level would be ambiguous
and cannot be interpreted consistently by tooling (e.g. loaders, validators, or
example generators).

For this reason, defining an `exampleValue` on a Property with an `Either`
Characteristic is explicitly disallowed.
