...
// What will happend if no timezone is provided? What will become a default timezone (or just ignore it)?
...
--
// From the diagram, readers would aspect to read from left to right, so the MultiLangaugeText box should appear to the right of the Text box, as Timestamp box is described first.
...
// This should come right after the MultiLanguageText box, Language, Locale, and then Boolean.
