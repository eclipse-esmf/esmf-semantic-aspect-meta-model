...
To ensure efficiency and maintainability, it is essential to reuse these elements across different Aspects
*_WHENEVER_* possible.
...
[[strict-ttl]]
== The rules of "Strict TTL"

To avoid potential issues when working with Aspects, it is best to adhere to stricter rules, known as "Strict TTL".
Below is a list of these rules:

* An aspect model file may only contain model elements of exactly one `xref:ROOT:modeling-guidelines.adoc#using-namespaces[namespace]`;

* Model element definitions of a `xref:ROOT:modeling-guidelines.adoc#using-namespaces[namespace]` may be spread across any number
of Aspect model files (i.e., the namespace's namespace files);

* A model element definition may only appear once per `xref:ROOT:modeling-guidelines.adoc#using-namespaces[namespace]` and aspect model file;

* A `xref:ROOT:modeling-guidelines.adoc#using-namespaces[namespace]` may contain any number of model elements, including `samm:Aspect`;

* An aspect model file may contain at most one model element definition for `samm:Aspect`;

* A namespace's set of `xref:ROOT:modeling-guidelines.adoc#using-namespaces[namespace]` files may contain at most
one `xref:ROOT:modeling-guidelines.adoc#using-namespaces[namespace]` element definition, which must match
the `xref:ROOT:modeling-guidelines.adoc#using-namespaces[namespace]`;
...
`models_root/package_part/version_part/aspect_model_file.ttl`;

* In filesystem-based model storage, the `xref:ROOT:modeling-guidelines.adoc#using-namespaces[namespace]` file containing an Aspect model element definition should have the same name as
the Aspect's local name suffixed with `".ttl"`.
