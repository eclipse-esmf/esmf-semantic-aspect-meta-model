...
All properties need to have meaningful descriptions that allow persons and AI-agents with no prior knowledge to understand them. This includes relevant usage scenarios, e.g.:
"This is the default property when grouping x by y."
If two properties are similar (e.g. "plantCode" and "plantId") the descriptions must be clear on when to use one or the other.
Do not just repeat the Preferred Name for the Description.
...
[[the-examplevalue-attribute]]
== The `exampleValue` attribute

The `exampleValue` attribute is available for Properties (samm:Property).
This attribute allows persons and AI-agents to quickly evaluate what correct values look like.

Properties always need to include an example value, ideally examples for all expected patterns are included (e.g. serialNumber might have values like "0123456" and also "000012ABCDE345678F").
...
does indeed mean something different.

In your area of responsibility, do not introduce Properties with the same local name and the same meaning in multiple namespaces.
For example, a Property "serialNumber" with a specific meaning must only be defined in exactly one namespace
(it can appear in multiple versions, though).
Concepts with different meanings may be declared with the same local name in different namespaces;
for example, an "operating system process" and an "organizational process" might both be called "process".
However, using more descriptive names than just "process", like in this example, is generally recommended.
