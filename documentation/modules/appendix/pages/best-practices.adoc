...
Do not introduce Properties with the same local name and the same meaning in multiple namespaces.
For example, a Property "serialNumber" with a specific meaning must only be defined in exactly one namespace
(it can appear in multiple versions, though). Concepts with different meanings may be declared with the same local name in different namespaces,
